...
public class MainAggregation {
    ...
    static public Table createPersonTable(String folder, String name, int pageSize, int size, boolean shuffled, int range, int cardinality) throws Exception {
        ...
        pt.addColumn(new StringColumn("nome", (short)30));
        pt.addColumn(new IntegerColumn("idade"));
        pt.addColumn(new StringColumn("cidade",(short)30));
    ...
    public static void main(String[] args) {
        try {
            ...
            if (recreate)
                table = createPersonTable("c:\\teste\\ibd", "tab1", Table.DEFULT_PAGE_SIZE, 100, false, 1, 50);
            else 
                table = Directory.getTable("c:\\teste\\ibd", "tab1",null, 9999, Table.DEFULT_PAGE_SIZE,  false);
        
            //create the query
            //Operation query = m.testGroupByQuery(table, "cidade", "idade", Aggregation.MIN);
            ...
            query = m.testGroupByQuery(table, "cidade", "idade", Aggregation.MAX);
            tuplesPrinter.execQueryAndPrint1(query, -1);
            query = m.testGroupByQuery(table, "cidade", "idade", Aggregation.SUM);
            tuplesPrinter.execQueryAndPrint1(query, -1);
            query = m.testGroupByQuery(table, "cidade", "idade", Aggregation.AVG);
            tuplesPrinter.execQueryAndPrint1(query, -1);
            query = m.testGroupByQuery(table, "cidade", "idade", Aggregation.COUNT);
            tuplesPrinter.execQueryAndPrint1(query, -1);
