...
public class TreePrinter {
    ...
    public void labelFunction(Operation o){
    ...
    /*
        ...
        drawing characters.
     */
    public void printTree(Operation root) {
    ...
    /*
        ...
        drawing characters.
     */
    public void printTrees(List<Operation> trees, int lineWidth) {
        ...
        for (int i = 0; i < trees.size(); i++) {
            Operation treeNode = trees.get(i);
    ...
    private List<TreeLine> buildTreeLines(Operation root) {
        if (root == null) return Collections.emptyList();
        else {
            String rootLabel = root.toString();
            
            List<TreeLine> leftTreeLines = null;
            List<TreeLine> rightTreeLines = null;

            if (root instanceof BinaryOperation){
                BinaryOperation bop = (BinaryOperation)root;
                leftTreeLines = buildTreeLines(bop.getLeftOperation());
                rightTreeLines = buildTreeLines(bop.getRightOperation());
            }
            else if (root instanceof UnaryOperation){
                UnaryOperation uop = (UnaryOperation)root;
                leftTreeLines = buildTreeLines(uop.getChildOperation());
                rightTreeLines = buildTreeLines(null);
            }
            else {
                SourceOperation sop = (SourceOperation)root;
                leftTreeLines = buildTreeLines(null);
                rightTreeLines = buildTreeLines(null);
            }
